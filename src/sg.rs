...
/// Structure with the alignement information of the contig and the alignment position.
pub struct Align{
    ref_len: u32,
    read_size: u32,
    reads: Vec<Read>,
...
pub fn extract_pairs(name: &String, bam_files_list: &String) ->  Align {
    ...
    for bam_file in bam_files_vec {
        ...
        for record in reader.fetch(&bam::Region::new(ref_id, 0, ref_len)).unwrap() {
            ...
            if flag.mate_is_mapped(){
                if flag.is_reverse_strand() && !flag.mate_is_reverse_strand(){
                    reads.push(Read{
                        position: u32::try_from(record.calculate_end()).unwrap() - 1,
                        forward: false,
                    });
                } else if !flag.is_reverse_strand() && flag.mate_is_reverse_strand() {
                    reads.push(Read{
    ...
    let align = Align{ ref_len, read_size, reads };  
    align
}
...
pub fn build_ratio(align: Align) -> PlusRatio {
    ...
    for (i, (forward, total)) in count_forward.iter().zip(&count_total).enumerate() {
    ...
    let ratios = PlusRatio{  position, count_forward, count_total, ratio };
    ratios
}
...
pub fn write_ratio(ratios: PlusRatio) -> Result<(), Box<dyn Error>> {
    ...
    for (pos, fw, tot, rat) in izip!(&ratios.position, &ratios.count_forward, &ratios.count_total, &ratios.ratio) {
        match rat {
            Some(rat) => wtr.write_record(&[pos.to_string(), fw.to_string(), tot.to_string(), rat.to_string()])?,
            None => wtr.write_record(&[pos.to_string(), fw.to_string(), tot.to_string(), "NA".to_string()])?,
        }
    } 
    // flush writer
    wtr.flush()?;
